...
export default function ServiceCard({ service, index, onOpenModal }: ServiceCardProps) {
  ...
  return (
    <motion.div variants={itemVariants}>
      <Card className="group hover:shadow-xl transition-all duration-300 hover:-translate-y-2 border-0 bg-white dark:bg-gray-800 h-full">
        <CardContent className="p-6 text-center h-full flex flex-col">
          ...
          <h3 className="text-lg font-semibold text-[#333333] dark:text-white mb-3 group-hover:text-[#FF6B35] transition-colors">
            {service.title}
          </h3>
          <p className="text-[#777777] dark:text-gray-300 text-sm leading-relaxed mb-4 flex-grow">
            {service.description}
          </p>
          <Button
            variant="outline"
